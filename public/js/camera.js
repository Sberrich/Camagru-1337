...
for(var i = 0; i < stickers.length; i++)
{
    stickers[i].onclick = function(event)
    {
        ...
        if(checker)
        {
            ...
            emoji = filters.src.replace("http://http://localhost/Camagru", "..");
        }
    }
}
///////////////////////Save Images ////////////////////////
save.addEventListener("click", function()
{
    if(canvas.toDataURL() !== document.getElementById('canvas2').toDataURL())
    {
        ...
        ajax.open('POST','http://http://localhost/camagru/posts/SaveImage');
...
for(var i=0; i < comment.length; i++){ 
  comment[i].onclick = function(event){
    ...
    var userid = (event.target && event.target.getAttribute('data-c-user_id'));
      if(userid == "")
      { window.location.replace("http://http://localhost/Camagru/users/login");
      }
    ...
    ajax.open('POST', 'http://http://localhost/Camagru/Posts/comment');
...
function like(event)
{
  ...
  var ajax = new XMLHttpRequest();
  ajax.open('POST', 'http://http://localhost/Camagru/Posts/Like');
  ajax.withCredentials = true;
